...
def post_install(directory):
    print_color(CYAN, f"{STAR}  Performing post install.\n")
    sources = ["deb http://http.kali.org/kali kali-rolling main contrib non-free",
               "deb-src http://http.kali.org/kali kali-rolling main contrib non-free"] 
    
    sourcesFile = f"{directory}/etc/apt/sources.list"
    try:
        with open(sourcesFile, 'w') as file:
            file.write('\n'.join(sources) + '\n')
        print_color(GREEN, f"{CHECK}  Post installation done.\n")

    except IOError as e:
        print_color(RED, f"{CROSS}  Post installation failed. Failed to modify sources.list: {e}.\n")
